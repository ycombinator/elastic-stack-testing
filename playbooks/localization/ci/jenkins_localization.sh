...
if [ $CI_BUILD == "true" ]; then
    ...
    if [ $ES_BUILD_OSS == "true" ]; then
      export AIT_ANSIBLE_PLAYBOOK="$(pwd)/playbooks/localization/install_oss_localization.yml"
    else
      export AIT_ANSIBLE_PLAYBOOK="$(pwd)/playbooks/localization/install_default_localization.yml"
    fi
